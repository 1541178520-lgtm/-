...
export function ScoreTimeline({ scores, onEdit, onDelete }: Props) {
  if (scores.length === 0) {
    return <div className="empty-chapter"><span aria-hidden="true">一</span><h4>尚无成绩记录</h4><p>添加一次重要考试，开始记录学生的成绩变化。</p></div>;
  }
  return (
      ...
      {scores.map((score, index) => (
        <li key={score.id}>
          ...
          <article className="score-card" data-testid={`score-${score.id}`}>
            ...
            <dl className="score-subjects">
              {SCORE_SUBJECTS.map(([key, label]) => score[key] === null ? null : (
                <div key={key}><dt>{label}</dt><dd>{score[key]}</dd></div>
              ))}
            </dl>
